...
| Demo (`demo.jerkai.app`) | same Vercel project as Production, host-based rewrite issued by `proxy.ts` to the `/demo` route subtree | none — reads only a committed synthetic fixture (`lib/demo/synthetic-data.ts`), never the database |

Preview deployments never touch production data.

The demo (`docs/prd/public-demo.md`) is a deliberately public, unauthenticated surface, rendering the real dashboard UI over synthetic data only. It structurally cannot reach the database — the `/demo` route tree imports no DB-touching module, a property checked by `tests/unit/demo-isolation.test.ts`, not just configured. Routing for the `demo.jerkai.app` host is handled entirely inside `proxy.ts` (both the session-gate exclusion for direct `/demo/*` paths and the host-based rewrite for the bare domain) rather than split across `proxy.ts` and a `next.config.ts` rewrite — Proxy always runs before any `next.config.js` rewrite in Next's execution order, so a rewrite declared there would never get a chance to fire once the session gate had already run on the original path. `jerkai.app/demo/results` and `/demo/daily` work without the custom domain; `demo.jerkai.app` additionally requires a one-time Vercel domain alias + DNS record (a manual step, not part of any deploy).

The demo also carries a static About surface at `demo.jerkai.app/about` (`/demo/about`, `docs/prd/demo-about.md`), linked from the demo nav: it explains what JerkAI is, that the data is synthetic, and that the write-side features are excluded by design rather than unfinished. It is a plain server component with no data import at all — not even the synthetic fixture — and the isolation guard enumerates the `app/demo` route segments from disk, so any demo route added later is covered by it automatically.

## Related repos

[`jerkai-mcp`](https://github.com/albimartai/jerkai-mcp) is a separate repo holding a local, read-only [Model Context Protocol](https://modelcontextprotocol.io) server that speaks JSON-RPC over stdio, so an MCP-connected chat client can ask what JerkAI tracks. It ships one tool today, `list_available_metrics`, which reports the biometric axes and the source system each comes from. It opens no database connection and holds no credential of any kind, and it reads no value out of `biometric_readings` — every coverage field it returns is `null` rather than a guess. What it does consume is this repo's metric registry: it carries byte-pinned copies of `lib/dashboard/types.ts` and `lib/dashboard/strain.ts` under `src/vendor/`, checked against a locked commit recorded in its own `vendor.lock.json`.

Those two files therefore have a consumer outside this repo. Changing either one breaks nothing here and fails no check in either repo; picking the change up is a deliberate step taken over in `jerkai-mcp` — re-copy the two files and bump the lock. See `docs/codebase.md` §6 for why the drift check does not catch this on its own.
