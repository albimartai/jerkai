...
* AC-M35 — Given create mode is holding some meal-type value V — **V is whatever the create-mode chip shows immediately before the user opens Edit, which may be its own time-of-day default (fresh mount) or a value the user has already manually clicked in create mode; V is not required to equal the time-of-day default**, exactly as a manual override on any other form field would persist through an unrelated action — When the user opens Edit on any entry (of a different meal type than V), edits, and then Cancels or Saves, Then create mode's value is still V once they return to it — the edited entry's meal type must not leak into or overwrite create mode's value, whatever that value happened to be. (Guards against an overcorrection where the new sync effect clobbers create-mode state on the way out of edit mode.) **Manual QA note (post-ship):** a test run that manually sets create mode to meal type X, edits an entry of type Y, cancels, and finds X restored is confirming this AC correctly — even if X is not the page's time-of-day default. Do not misread "X survived" as a failure because X ≠ `defaultMealType()`'s output; that would be testing AC-M34 (the mount-time default) against AC-M35's fixture (whatever create mode holds at the moment), which are two different guarantees. **This guarantee is verified, not newly built**: it depends entirely on the existing parent contract in `LogMealPanel` — `onEditComplete` sets `editEntry` back to `null`, at which point the new effect's `if (editEntry)` guard is false and it does nothing (see IN-2). Do not add "reset on exit" logic to satisfy this AC; the test confirms the existing contract holds, it does not exercise new code.
