...
// incoming path, which for a demo.jerkai.app visitor is "/" (or "/body",
// "/daily"), not "/demo/body", so those paths are NOT excluded by the
...
describe("proxy host bypass for demo.jerkai.app", () => {
  it("AC-W27: rewrites demo.jerkai.app/ to /demo/body without calling auth() (regression: used to 307 to jerkai.app/signin)", async () => {
    authMock.mockClear();
    const response = await callProxy("https://demo.jerkai.app/");
    expect(authMock).not.toHaveBeenCalled();
    expect(isRewrite(response)).toBe(true);
    expect(getRewrittenUrl(response)).toBe("https://demo.jerkai.app/demo/body");
  });
  ...
  it("AC-W27: rewrites for demo.jerkai.app with an explicit port (local/proxy testing)", async () => {
    authMock.mockClear();
    const response = await callProxy("http://demo.jerkai.app:3000/");
    expect(authMock).not.toHaveBeenCalled();
    expect(isRewrite(response)).toBe(true);
    expect(getRewrittenUrl(response)).toBe("http://demo.jerkai.app:3000/demo/body");
  });
});
