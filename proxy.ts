...
// app/page.tsx and app/connect/page.tsx. (app/status/page.tsx and
// app/data/page.tsx are now both just redirect("/connect") stubs, Rename
// /data Page to /connect AC-DS23/AC-DS24 — neither performs that re-check
// itself.)
//
...
// visitor typing "demo.jerkai.app" requests path "/" (or "/body",
// "/daily"), not "/demo/body" — that path doesn't match the demo(?:$|/)
// matcher exclusion, so proxy() DOES run for it, and a next.config.ts
// rewrite from "/" to "/demo/body" would never get a chance to fire
...
export default function proxy(request: NextRequest, event: NextFetchEvent) {
  if (isDemoHost(request.headers.get("host") ?? "")) {
    ...
    url.pathname = url.pathname === "/" ? "/demo/body" : `/demo${url.pathname}`;
