...
// a two-parameter function over three series only — strainRaw/recoveryRaw no
// longer exist on LedgerInput, and buildWeeklyLedger no longer takes a
// recoveryCfg argument. The former "strain and recovery levels (AC-W3)"
// describe block is retired outright (AC-W36) — the behavior it tested no
// longer exists.
...
describe("buildWeeklyLedger — week keys and shape (AC-W1, AC-W5)", () => {
  it("AC-W1: rows use ISO Mon–Sun week keys, newest first", () => {
    const rows = buildWeeklyLedger(fixture(3, 0), cfg);
  ...
  it("AC-W1: a partial current week is its own in-progress row, days-elapsed labeled, never marked complete", () => {
    const rows = buildWeeklyLedger(fixture(2, 4), cfg);
  ...
  it("AC-W5: completed weeks are capped at cfg.maxCompletedWeeks, newest first", () => {
    const rows = buildWeeklyLedger(fixture(5, 0), { ...cfg, maxCompletedWeeks: 2 });
...
describe("buildWeeklyLedger — delta convention (AC-W2)", () => {
  it("AC-W2: a noisy raw series with a flat smoothed trend produces ~0 delta, not a noise-driven one", () => {
    ...
    const rows = buildWeeklyLedger(fixture(2, 0, { bodyFatRaw, bodyFat30 }), cfg);
    const [newest] = rows;
  ...
  it("AC-W2/AC-W4: a body-fat drop beyond epsilon reads good; a rise beyond epsilon reads warning", () => {
    ...
    const rows = buildWeeklyLedger(fixture(2, 0, { bodyFat30 }), cfg);
  ...
  it("the first week in history has no prior week to compare against, so its delta cell is insufficient", () => {
    const rows = buildWeeklyLedger(fixture(1, 0), cfg);
...
describe("buildWeeklyLedger — lean mass weekly band (AC-W3, AC-W4)", () => {
  it("a lean-mass drop beyond the weekly band reads warning; within it reads neutral", () => {
    ...
    const rows = buildWeeklyLedger(fixture(2, 0, { leanMass7: droppingLbm }), cfg);
    ...
    const holdingRows = buildWeeklyLedger(fixture(2, 0, { leanMass7: holdingLbm }), cfg);
...
describe("buildWeeklyLedger — sparse and empty weeks (AC-W7)", () => {
  it("a week with fewer than minDaysPerWeek raw readings for a series renders insufficient data for that column", () => {
    ...
    const rows = buildWeeklyLedger(fixture(1, 0, { bodyFatRaw }), cfg);
    expect(rows[0].columns.bodyFat).toEqual({ kind: "insufficient", daysPresent: 2 });
  });

  it("a week with zero data across every series collapses to a single gap row, not three insufficient cells", () => {
    const axis = axisOf(MONDAY, 14);
...
describe("buildWeeklyLedger — gap-week detection narrows to three series (AC-W33)", () => {
  it("AC-W33 (bare case): a week with body-fat, weight, or lean-mass data is not a gap row", () => {
    const rows = buildWeeklyLedger(fixture(1, 0), cfg);
    expect(rows[0].isGap).toBe(false);
  });

  it("AC-W33: a week with no body-fat, weight, or lean-mass data collapses to a gap row — the reverse of today's shipped behavior, where Strain/Recovery presence alone would have kept it from collapsing", () => {
    const axis = axisOf(MONDAY, 7);
...
describe("buildWeeklyLedger — cold start (AC-W9)", () => {
  it("with fewer than 2 completed weeks, returns the available rows without erroring", () => {
    const rows = buildWeeklyLedger(fixture(1, 3), cfg);
    expect(() => buildWeeklyLedger(fixture(1, 3), cfg)).not.toThrow();
  ...
  it("with an empty axis, returns no rows and never errors", () => {
    expect(buildWeeklyLedger(fixture(0, 0), cfg)).toEqual(
      buildWeeklyLedger({ ...fixture(0, 0), axis: [] }, cfg),
    );
    expect(buildWeeklyLedger({ ...fixture(0, 0), axis: [] }, cfg)).toEqual([]);
  });
});
